...
void displayWAVHDR (WAVHDR hdr){
	...
	gotoXY(1,15); setFGcolor(CYAN); printf("%d bit/s", hdr.BitsPerSample);
	gotoXY(1,30); setFGcolor(YELLOW); printf("%d sps", hdr.SampleRate);
	resetColors();
#endif
}
...
// function displayWAVdata calculates 1-sec samples into 80 pieces
